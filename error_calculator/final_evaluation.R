...
rnn_forecast_file_path = "results/rnn_forecasts/temperature_stacking_LSTMcell_moving_window_without_stl_decomposition_smac_adam_without_truncated_backpropagation_without_accumulated_error_1.txt"
errors_file_name = "temperature_stacking_LSTMcell_moving_window_without_stl_decomposition_smac_adam_without_truncated_backpropagation_without_accumulated_error_1.txt"
txt_test_file_name = "datasets/text_data/moving_window/without_stl_decomposition/temperature_test_12i120.txt"
actual_results_file_name = "datasets/text_data/temperature_test_data.csv"
original_data_file_name = "datasets/text_data/temperature_original_data.csv"
input_size = 120
output_size = 12
...
root_directory = paste("/home/rakshitha/PycharmProjects", "temperature-forecasting", sep = "/")
...
actual_results = read.csv(file = actual_results_file_full_name)
...
original_dataset <- read.csv(file = original_data_file_full_name)
original_dataset <- as.data.frame(original_dataset[,-1])
colnames(original_dataset)[1] <- "x"
original_dataset <- t(original_dataset)
...
actual_results_df <- actual_results[colSums(is.na(actual_results)) == 0,]
actual_results_df <- as.data.frame(actual_results_df[,-1])
colnames(actual_results_df)[1] <- "x"
actual_results_df <- t(actual_results_df)

converted_forecasts_df = NULL
converted_forecasts_matrix = matrix(nrow = nrow(forecasts_df), ncol = output_size)  # Rows = no: of time series, columns = forecast horizon
